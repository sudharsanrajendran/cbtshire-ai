...
import { createCandidate, resendAssessmentLink, parseCandidateResume, getCandidates, type ResendAssessmentResult } from '../services/candidateService';
...
export function ModulePage({ kind }: { kind: ModuleKind }) {
  ...
  const [autoGenerateDesc, setAutoGenerateDesc] = useState(false);
  ...
  // Handle on-demand market skills suggestion for given Job Title / Role
  ...
  const toggleSkill = (skillName: string) => {
    ...
    if (exists) {
      nextSkills = currentSkills.filter((s) => s.toLowerCase() !== skillName.toLowerCase());
    } else {
      nextSkills = [...currentSkills, skillName];
    }
    setForm((cur) => ({ ...cur, skills: nextSkills.join(', ') }));
  };
  ...
  const [candidateFilterRole, setCandidateFilterRole] = useState<string>('all');
  const [candidateFilterShortlist, setCandidateFilterShortlist] = useState<boolean>(false);
  const [bulkSending, setBulkSending] = useState<boolean>(false);
  const [bulkSendMsg, setBulkSendMsg] = useState<string | null>(null);

  // Candidate Profile & Resume Review Modal state
  const [selectedCandidateForReview, setSelectedCandidateForReview] = useState<Candidate | null>(null);
  const [candidateDetailOpen, setCandidateDetailOpen] = useState<boolean>(false);
  const [activeCandidateTab, setActiveCandidateTab] = useState<'profile' | 'resume'>('profile');
  const [copiedResumeText, setCopiedResumeText] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (kind === 'jobs') {
      void getCandidates().then(setAvailableCandidates).catch(() => undefined);
    }
  }, [kind, items]);

  // Bulk send assessment to all candidates with match_score >= 70
  const handleBulkSendShortlisted = async () => {
    if (kind !== 'candidates') return;
    const candidatesList = items as Candidate[];
    const toSend = candidatesList.filter(c => (c.match_score || 0) >= 70 && c.status !== 'Assessment Sent');
    ...
    try {
      for (const cand of toSend) {
        try {
          await resendAssessmentLink(cand.id);
          sentCount++;
        } catch (e) {
          console.error('Failed to send assessment to candidate:', cand.id, e);
        }
      }
      setBulkSendMsg(`Successfully emailed AI assessment invitations to ${sentCount} shortlisted candidates (>70% match)!`);
      await reload();
    } catch (err: any) {
      console.error(err);
      setBulkSendMsg('Encountered an issue while sending bulk assessments.');
    } finally {
  ...
  const displayedItems = useMemo(() => {
    if (kind !== 'candidates') return items;
    return (items as Candidate[]).filter((c) => {
      if (candidateFilterShortlist && (c.match_score || 0) < 70) return false;
      if (candidateFilterRole !== 'all' && c.role !== candidateFilterRole) return false;
      return true;
    });
  }, [items, kind, candidateFilterShortlist, candidateFilterRole]);
  ...
  return (
    <Stack spacing={3}>
      ...
      {bulkSendMsg && <Alert severity="success" onClose={() => setBulkSendMsg(null)}>{bulkSendMsg}</Alert>}

      {/* Candidate AI Shortlisting & Role Statistics Toolbar */}
      {kind === 'candidates' && items.length > 0 && (
        <Paper elevation={0} sx={{ p: 2.2, borderRadius: 3, border: '1px solid', borderColor: 'divider', bgcolor: '#f8fafc' }}>
          <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'center' }} gap={2} flexWrap="wrap">
            <Stack direction="row" spacing={1.2} alignItems="center" flexWrap="wrap">
              <Chip
                icon={<PeopleRoundedIcon />}
                label={`All Applicants (${(items as Candidate[]).length})`}
                color={!candidateFilterShortlist && candidateFilterRole === 'all' ? 'primary' : 'default'}
                variant={!candidateFilterShortlist && candidateFilterRole === 'all' ? 'filled' : 'outlined'}
                onClick={() => { setCandidateFilterShortlist(false); setCandidateFilterRole('all'); }}
                sx={{ fontWeight: 700, cursor: 'pointer' }}
              />
              <Chip
                icon={<AutoAwesomeRoundedIcon sx={{ color: '#059669 !important' }} />}
                label={`✨ AI Shortlisted >70% (${(items as Candidate[]).filter(c => (c.match_score || 0) >= 70).length})`}
                sx={{
                  fontWeight: 800,
                  cursor: 'pointer',
                  bgcolor: candidateFilterShortlist ? '#dcfce7' : '#f0fdf4',
                  color: '#15803d',
                  borderColor: '#86efac',
                  border: '1px solid'
                }}
                onClick={() => setCandidateFilterShortlist(!candidateFilterShortlist)}
              />
              <FormControl size="small" sx={{ minWidth: 220 }}>
                <InputLabel id="role-filter-label">Filter by Job Profile</InputLabel>
                <Select
                  labelId="role-filter-label"
                  value={candidateFilterRole}
                  label="Filter by Job Profile"
                  onChange={(e) => setCandidateFilterRole(e.target.value)}
                  sx={{ bgcolor: 'white', borderRadius: 2 }}
                >
                  <MenuItem value="all">All Profiles ({(items as Candidate[]).length})</MenuItem>
                  {Array.from(new Set((items as Candidate[]).map(c => c.role).filter(Boolean))).map(role => {
                    const count = (items as Candidate[]).filter(c => c.role === role).length;
                    const shortlisted = (items as Candidate[]).filter(c => c.role === role && (c.match_score || 0) >= 70).length;
                    return (
                      <MenuItem key={role} value={role}>
                        {`${role} (${count} applied · ${shortlisted} >70%)`}
                      </MenuItem>
                    );
                  })}
                </Select>
            ...
            <Button
              variant="contained"
              color="success"
              startIcon={bulkSending ? <CircularProgress size={16} color="inherit" /> : <SendRoundedIcon />}
              onClick={handleBulkSendShortlisted}
              disabled={bulkSending || (items as Candidate[]).filter(c => (c.match_score || 0) >= 70 && c.status !== 'Assessment Sent').length === 0}
              sx={{ fontWeight: 800, borderRadius: 2, bgcolor: '#059669', '&:hover': { bgcolor: '#047857' } }}
            >
              {`🚀 Send Assessment to All >70% (${(items as Candidate[]).filter(c => (c.match_score || 0) >= 70 && c.status !== 'Assessment Sent').length})`}
            </Button>
          </Stack>
        </Paper>
      )}
      ...
      {loading ? (
        ...
        <Card>
          <CardContent sx={{ py: 8, textAlign: 'center' }}>
            <Typography variant="h5">No {title.toLowerCase()} match filter</Typography>
            <Typography color="text.secondary" sx={{ mt: 1 }}>
              Try changing or resetting your filter criteria above.
            </Typography>
          </CardContent>
        </Card>
      ) : (
          ...
          {displayedItems.map((item) => (
            <Card
              ...
              sx={{
                borderLeft:
                  kind === 'candidates'
                    ? ((item as Candidate).match_score || 0) >= 70
                      ? '5px solid #10b981'
                      : '5px solid #e2e8f0'
                    : undefined
              }}
              ...
              <CardContent>
                <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} gap={2}>
                  {kind === 'jobs' && (() => {
                    ...
                    const applicantCount = (availableCandidates || []).filter(c => c.role === jobItem.title).length;
                    const shortlistedCount = (availableCandidates || []).filter(c => c.role === jobItem.title && (c.match_score || 0) >= 70).length;
                    return (
                      <Box>
                        <Typography fontWeight={800}>{jobItem.title}</Typography>
                        <Typography variant="body2" color="text.secondary">
                          {jobItem.department} · {jobItem.location}
                        </Typography>
                        <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 0.8 }}>
                          <Chip
                            icon={<PeopleRoundedIcon sx={{ fontSize: '14px !important' }} />}
                            label={`${applicantCount} Applied`}
                            size="small"
                            sx={{ fontWeight: 700, fontSize: 11, bgcolor: '#f1f5f9' }}
                          />
                          <Chip
                            icon={<AutoAwesomeRoundedIcon sx={{ fontSize: '14px !important', color: '#059669 !important' }} />}
                            label={`${shortlistedCount} Shortlisted (>70%)`}
                            size="small"
                  ...
                  {kind === 'candidates' && (() => {
                    ...
                    const isShortlisted = (cand.match_score || 0) >= 70;
                    return (
                      <Box>
                        <Stack direction="row" alignItems="center" spacing={1.2} flexWrap="wrap">
                          <Typography fontWeight={800} sx={{ fontSize: 16 }}>{cand.name}</Typography>
                          {renderPlatformSourceBadge(cand.source)}
                        </Stack>
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.3 }}>
                          Applied for: <strong style={{ color: '#0284c7' }}>{cand.role}</strong> · {cand.email}
                        </Typography>
                        <Stack direction="row" spacing={0.8} alignItems="center" flexWrap="wrap" sx={{ mt: 0.8, gap: 0.5 }}>
                          {isShortlisted ? (
                            <Chip
                              icon={<AutoAwesomeRoundedIcon sx={{ fontSize: '14px !important', color: '#047857 !important' }} />}
                              label={`✨ AI Shortlisted (${cand.match_score}% Match · Highly Suitable)`}
                              size="small"
                              sx={{ fontWeight: 800, fontSize: 11, bgcolor: '#ecfdf5', color: '#047857', border: '1px solid #6ee7b7' }}
                            />
                          ) : cand.match_score && cand.match_score > 0 ? (
                            <Chip
                              label={`⚠️ ${cand.match_score}% Match (Needs Review)`}
                              size="small"
                              sx={{ fontWeight: 700, fontSize: 11, bgcolor: '#fffbeb', color: '#b45309', border: '1px solid #fde68a' }}
                            />
                          ) : null}

                          {cand.experience_level && (
                            <Chip label={cand.experience_level} size="small" variant="outlined" sx={{ fontWeight: 700, fontSize: 11, bgcolor: '#f0fdf4', borderColor: '#86efac', color: '#166534' }} />
                          )}
                    ...
                    {kind === 'candidates' && (
                      <Stack direction="row" spacing={1} flexWrap="wrap">
                        <Button
                          ...
                          onClick={() => {
                            setSelectedCandidateForReview(item as Candidate);
                            setActiveCandidateTab('profile');
                            setCandidateDetailOpen(true);
                          }}
                          sx={{ fontWeight: 700, borderRadius: 2 }}
                        >
                          👁️ View Resume & Profile
                        </Button>
                        <Button
                          size="small"
                          variant="contained"
